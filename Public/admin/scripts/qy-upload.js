$.fn.extend({
  qy_upload:function(action,name,multiple){
      ...
      $(this).parent().ajaxSubmit({
        ...
        success: function(data) { //成功
          if(data.status == 1){
            ...
            //判断是否上传多张图片
            if(multiple == 0){
              thisObj.parent().before('<li><img src="/Uploads/'+data.path+data.pic+'" /><input type="hidden" name="'+name+'" value="'+data.path+data.pic+'" /><span class="qy-upload-del"><i class="fa fa-remove"></i></span></li>');
              thisObj.parent().hide();
            }else{
              thisObj.parent().before('<li><img src="/Uploads/'+data.path+data.pic+'" /><input type="hidden" name="'+name+'[]" value="'+data.path+data.pic+'" /><span class="qy-upload-del"><i class="fa fa-remove"></i></span></li>');
            }
    ...
    $(".qy-upload").on('click','.qy-upload-del',function(){
      if(multiple == 1){
        ...
        $.post("/index.php?m=Admin&c=Upload&a=del",{filename:filename,},function(data,status){
          if(status == "success"){
            if(data == "1"){
              this_file.remove();
