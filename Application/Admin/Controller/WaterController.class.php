 ...
 * @author @janpun
 */
class WaterController extends CommonController{
  ...
  public function water() {
    ...
    $count = M('Shuiliao') -> count();
    $Pages = new \Think\Page($count,25);// 实例化分页类 传入总记录数和每页显示的记录数(25)
    $show = $Pages->show();// 分页显示输出
    $data = M('Shuiliao') -> order('id desc') -> limit($Page -> firstRow.','.$Page -> listRows) -> select();
  ...
  public function add_picture() {
    if(IS_POST){
      ...
      $count = M('Shuiliao') -> add($map);

      if($count > 0){
        $this -> success("添加成功", U('water'));
      }else{
        $this -> error("添加失败");
      }
    }else{
  ...
  public function edit_picture() {
    if(IS_POST){
      ...
      $map['time'] = strtotime($post['time']);
      // var_dump($map);exit;
      $count = M('Shuiliao') -> save($map);

      if($count > 0){
        $this -> success("编辑成功", u('water'));
      }else{
        $this -> error("编辑失败");
      }
    }else{
