...
class WaterController extends GlobalController {
  public function index(){
    // $title = "index_logo";
    $_SESSION['aa'] = 1;
  //获取数据
  // $info = D('Block') -> get_item_by_title('about_logo');
  $about = D('Block') -> get_item_by_title('Water_index');
  ...
  // $data['content'] = html_entity_decode($data['content']);
  // var_dump($data);exit;
    $shuiliao = M('Shuiliao') -> order('id desc') -> select();
   
    $result = array();
    foreach ($shuiliao as $value){
